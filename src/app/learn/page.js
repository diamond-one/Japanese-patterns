...
export default function LearnPage() {
    ...
    useEffect(() => {
        const savedProgress = localStorage.getItem('japanese-app-progress-v1');
        if (savedProgress) {
            setProgress(JSON.parse(savedProgress));
        }

        // Streak Logic: Check/Reset on load
        const streakData = JSON.parse(localStorage.getItem('japanese-app-streak') || '{"count": 0, "lastDate": ""}');
    ...
    const saveProgress = (newProgress) => {
        ...
        localStorage.setItem('japanese-app-progress-v1', JSON.stringify(newProgress));
    };
    ...
    const handleRecording = () => {
        ...
        if (streakData.lastDate !== today) {
            ...
            setStreak(newCount);
            localStorage.setItem('japanese-app-streak', JSON.stringify({ count: newCount, lastDate: today }));
        }
        // ---------------------
